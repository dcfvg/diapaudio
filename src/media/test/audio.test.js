...
describe("audio", () => {
  ...
  describe("loadAllAudioDurations", () => {
    ...
    it("limits concurrent metadata loads", async () => {
      ...
      const loadPromise = loadAllAudioDurations(tracks, { maxConcurrency: 2 });

      expect(audioInstances).toHaveLength(2);

      const resolveNextPending = async (duration) => {
        const pending = audioEventHandlers.find(
          ({ audio, event }) => event === "loadedmetadata" && !audio.__resolved
        );
        pending.audio.__resolved = true;
        pending.audio.duration = duration;
        pending.handler();
        await Promise.resolve();
      };

      await resolveNextPending(10);
      expect(audioInstances).toHaveLength(3);

      await resolveNextPending(20);
      expect(audioInstances).toHaveLength(4);

      await resolveNextPending(30);
      expect(audioInstances).toHaveLength(5);

      await resolveNextPending(40);
      await resolveNextPending(50);
      await loadPromise;

      expect(tracks.map((track) => track.duration)).toEqual([10, 20, 30, 40, 50]);
    });

    it("times out hung metadata loads and continues the queue", async () => {
      vi.useFakeTimers();
      ...
      const loadPromise = loadAllAudioDurations(tracks, { maxConcurrency: 1, timeoutMs: 50 });
      ...
      const secondLoaded = audioEventHandlers.find(
        ({ audio, event }) => event === "loadedmetadata" && audio === audioInstances[1]
      );
      secondLoaded.audio.duration = 123;
      secondLoaded.handler();

      await loadPromise;
      expect(tracks[1].duration).toBe(123);
